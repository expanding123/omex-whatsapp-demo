...
function bindForm(){
  $('#omex-form').on('submit',async e=>{
    ...
    try{
      ...
      showErr('Error: '+err.statusText+' - Verifica que el servidor Render este activo.');
      console.error('OmexDemo error:',err);
    }
  });
}
function connectSocket(sid){
  if(socket)socket.disconnect();
  if(!C.socketUrl){showErr('Socket URL no configurada.');return;}
  socket=io(C.socketUrl,{transports:['websocket','polling'],timeout:20000});
  socket.on('connect',()=>{console.log('Socket conectado');socket.emit('subscribe',{session_id:sid});});
  socket.on('connect_error',err=>{console.error('Socket error:',err);showErr('No se pudo conectar al servidor. Intentando...');});
...
function startTTL(){
  ttlLeft=(C.ttlMin||30)*60;updateTTL();
...
function showErr(msg){$('#omex-error').text(msg).show();step('form');}
